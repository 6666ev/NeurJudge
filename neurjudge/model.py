...
class NeurJudge(nn.Module):
    def __init__(self,embeddings):
        ...
        self.word2id = json.load(open('data/laic/word2id.json'))
        ...
        self.article_num = len(self.article2id)

        self.embs = nn.Embedding(self.vocab_size, self.emb_dim)
        
        embeddings = torch.Tensor(embeddings)
        self.embs.weight.data.copy_(embeddings)
        self.embs.weight.requires_grad = False

        self.encoder = nn.GRU(self.emb_dim,self.hidden_dim, batch_first=True, bidirectional=True)
        ...
        self.encoder_charge = nn.GRU(self.emb_dim,self.hidden_dim, batch_first=True, bidirectional=True)

        self.charge_pred = nn.Linear(self.hidden_dim*6,self.charge_num)
        self.article_pred = nn.Linear(self.hidden_dim*8,self.article_num)
        self.time_pred = nn.Linear(self.hidden_dim*6,1)
    ...
    documents,sent_lent,process,device):
        ...
        new_article = self.graph_decomposition_operation(_label = _article, label_sim = self.art_tong, id2label = id2art, label2id = art2id, num_label = self.article_num, layers = 2)
